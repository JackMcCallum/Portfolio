...
uniform vec4 uLightPosition; // w == distance
...
void main()
{
	...
	vec3 L = normalize(uLightPosition.xyz - worldPos);
	vec3 N = texture(uTexNormals, vTexCoord).xyz*2-1;
	vec3 E = normalize(uCameraPosition - worldPos);
	
	float Attenuation = pow(max(0, 1-(length(uLightPosition.xyz - worldPos)) / uLightPosition.w), 2);
	if (Attenuation == 0) discard;
	
	float D, S;
	orenNayarCookTorrance(N, L, E, 0.3, 0.5, D, S);
	
	oColor = vec4(vec3(D+S)*uLightColour,0) * diffSamp * Attenuation;
}